                    ...
                    <div className="flex flex-wrap items-center justify-between py-3 gap-y-3 relative">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="hidden sm:block">
                                <h1 className="text-sm font-black tracking-wide uppercase leading-none text-white">Sunday Game</h1>
                                <span className="text-[10px] font-bold text-[#FFD166] uppercase tracking-widest mt-1 block">Cổng Nhân Viên</span>
                            </div>
                        </div>
                        ...
                        <div className="w-full md:w-auto order-last md:order-none flex justify-center">
                            <div className="flex items-center bg-white/10 p-1 rounded-xl shadow-inner w-full sm:w-auto">
                                {[
                                    { id: 'schedule', icon: <CalendarCheck size={16}/>, label: 'Lịch Làm Việc' },
                                    { id: 'salary', icon: <CircleDollarSign size={16}/>, label: 'Lương Cá Nhân' }
                                ].map(tab => (
                                    <button 
                                        key={tab.id}
                                        onClick={() => setActiveTab(tab.id)}
                                        className={`flex-1 sm:flex-none min-w-[130px] px-3 sm:px-6 py-2.5 rounded-lg text-xs sm:text-sm font-bold flex items-center justify-center gap-2 transition-all duration-200 ${
                                            activeTab === tab.id ? 'bg-[#FFD166] text-[#0B1E3F] shadow-md scale-[1.02]' : 'text-gray-300 hover:text-white hover:bg-white/20'
                                        }`}
                                    >
                        ...
                        <div className="flex items-center gap-3 sm:gap-4 relative" ref={menuRef}>
                            <div className="text-right hidden sm:block">
                                ...
                                <p className="text-xs font-extrabold text-[#FFD166] leading-none">{user?.ho_ten || localStorage.getItem('ho_ten')}</p>
                            </div>
                            <div className="h-8 w-px bg-white/20 hidden sm:block"></div>
                            
                            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className={`p-2.5 rounded-xl flex items-center gap-2 transition-colors border shadow-sm ${isMenuOpen ? 'bg-[#FFD166] border-[#FFD166] text-[#0B1E3F]' : 'bg-white/10 border-white/20 hover:bg-white/20 text-white'}`}>
                            ...
                            {isMenuOpen && (
                                <div className="absolute top-[120%] right-0 w-56 bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden animate-fade-in-up origin-top-right z-50">
                                    ...
                                    <div className="p-1.5">
                                        <button onClick={openProfileModal} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-gray-700 hover:bg-blue-50 hover:text-blue-700 rounded-lg transition"><User size={16} /> Thông tin tài khoản</button>
                                        <button onClick={openPasswordModal} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-gray-700 hover:bg-amber-50 hover:text-amber-700 rounded-lg transition"><Key size={16} /> Đổi mật khẩu</button>
                                    </div>
                                    <div className="p-1.5 border-t border-gray-100 bg-gray-50">
                                        <button onClick={() => { setIsMenuOpen(false); if(window.confirm("Bạn muốn đăng xuất?")) logout(); }} className="w-full flex items-center justify-center gap-2 px-3 py-2.5 text-sm font-bold text-red-600 hover:bg-red-100 hover:text-red-700 rounded-lg transition"><LogOut size={16} strokeWidth={2.5} /> Đăng xuất</button>
                                    </div>
                                </div>
                            )}
                        </div>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-fade-in-up">
                        <div className="p-4 bg-[#0B1E3F] text-white flex justify-between items-center">
                            <h3 className="font-bold uppercase tracking-wider text-sm flex items-center gap-2"><User size={18} className="text-[#FFD166]"/> Thông tin tài khoản</h3>
                        ...
                        <form onSubmit={handleUpdateProfile} className="p-5 space-y-4 bg-gray-50">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Mã NV (Chỉ xem)</label>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-fade-in-up">
                        <div className="p-4 bg-amber-600 text-white flex justify-between items-center">
                            <h3 className="font-bold uppercase tracking-wider text-sm flex items-center gap-2"><Key size={18} /> Đổi mật khẩu</h3>
