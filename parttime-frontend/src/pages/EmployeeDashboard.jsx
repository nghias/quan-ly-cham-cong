...
import { LogOut, CalendarCheck, CircleDollarSign, StickyNote, ChevronLeft, ChevronRight, Menu, User, Key } from 'lucide-react';
...
import NotesTab from '../components/common/NotesTab'; // Import Tab Ghi chú
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-20 relative">
                        ...
                        <div className="w-full md:w-auto order-last md:order-none flex justify-center">
                            <div className="flex items-center bg-white/10 p-1 rounded-xl shadow-inner w-full sm:w-auto overflow-x-auto hide-scrollbar">
                                {[
                                    ...
                                    { id: 'notes', icon: <StickyNote size={16}/>, label: 'Ghi Chú' } // <--- TAB MỚI Ở ĐÂY
                                ].map(tab => (
                                    <button 
                                        key={tab.id}
                                        onClick={() => setActiveTab(tab.id)}
                                        className={`flex-1 sm:flex-none min-w-[100px] px-3 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-bold flex items-center justify-center gap-2 transition-all duration-200 ${
                                ...
                                <div className="absolute top-[120%] right-0 w-56 bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden animate-fade-in-up origin-top-right z-50">
                                    ...
                                    <div className="p-1.5">
                                        <button 
                                            ...
                                            className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-gray-700 hover:bg-blue-50 hover:text-blue-700 rounded-lg transition cursor-pointer"
                                        >
                                            <User size={16} /> Thông tin tài khoản
                                        </button>
                                        <button 
                                            onClick={() => { setIsMenuOpen(false); setIsPasswordModalOpen(true); }} 
                                            className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-gray-700 hover:bg-amber-50 hover:text-amber-700 rounded-lg transition cursor-pointer"
                                        >
                                    ...
                                    <div className="p-1.5 border-t border-gray-100 bg-gray-50">
                                        <button 
                                            onClick={() => { setIsMenuOpen(false); if(window.confirm("Bạn muốn đăng xuất?")) logout(); }} 
                                            className="w-full flex items-center justify-center gap-2 px-3 py-2.5 text-sm font-bold text-red-600 hover:bg-red-100 hover:text-red-700 rounded-lg transition cursor-pointer"
                                        >
            ...
            {activeTab === 'schedule' && (
                <div className="max-w-[1920px] mx-auto px-4 mt-6 flex justify-center items-center gap-3 sm:gap-4">
                    <button 
                        ...
                        className="p-2 sm:p-2.5 bg-white border border-gray-200 rounded-full shadow-sm disabled:opacity-40 hover:bg-gray-50 transition-colors cursor-pointer"
                    >
            ...
            <main className="max-w-[1920px] mx-auto px-2 sm:px-4 mt-6 space-y-6">
                ...
                {activeTab === 'notes' && <NotesTab />} {/* GỌI COMPONENT MỚI Ở ĐÂY */}
            </main>
